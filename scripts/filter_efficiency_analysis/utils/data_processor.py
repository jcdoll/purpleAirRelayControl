...
class DataProcessor:
    ...
    def _calculate_building_params(self) -> Dict[str, float]:
        ...
        params = {'volume': volume_m3, 'filtration_rate': filtration_rate_m3h, 'deposition_rate': deposition_rate_m3h}

        self.logger.info(f"Building parameters: {params}")
        return params

    def filter_night_time_data(self, df: pd.DataFrame, timestamp_col: str = 'timestamp') -> pd.DataFrame:
        """
    ...
    def convert_aqi_columns(
        self, df: pd.DataFrame, indoor_aqi_col: str = 'indoor_aqi', outdoor_aqi_col: str = 'outdoor_aqi'
    ) -> pd.DataFrame:
        """
        ...
        if indoor_aqi_col in df.columns:
            df['indoor_pm25'] = df[indoor_aqi_col].apply(lambda x: aqi_to_pm25(x) if pd.notna(x) and x >= 0 else np.nan)
    ...
    def calculate_io_ratio(
        self, df: pd.DataFrame, indoor_col: str = 'indoor_pm25', outdoor_col: str = 'outdoor_pm25'
    ) -> pd.DataFrame:
        """
    ...
    def prepare_model_data(
        ...
        valid_mask = (
            pd.notna(df[indoor_col]) & pd.notna(df[outdoor_col]) & (df[indoor_col] >= 0) & (df[outdoor_col] > 0)
        )
        ...
        indoor_values = np.array(clean_df[indoor_col])
        outdoor_values = np.array(clean_df[outdoor_col])
        timestamps = np.array(clean_df[timestamp_col])
