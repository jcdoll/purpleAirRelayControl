...
def aqi_to_pm25(aqi: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    if isinstance(aqi, (int, float)):
        ...
        return np.array([convert_single_aqi(float(val)) if not np.isnan(val) else np.nan 
                        for val in aqi.flatten()]).reshape(aqi.shape)
    else:
        # Try to convert to array and process
        aqi_array = np.asarray(aqi)
        return np.array([convert_single_aqi(float(val)) if not np.isnan(val) else np.nan 
                        for val in aqi_array.flatten()]).reshape(aqi_array.shape)
...
def pm25_to_aqi(pm25: Union[float, np.ndarray]) -> Union[int, np.ndarray]:
    ...
    if isinstance(pm25, (int, float)):
        ...
        return np.array([convert_single_pm25(float(val)) if not np.isnan(val) else np.nan 
                        for val in pm25.flatten()]).reshape(pm25.shape)
    else:
        # Try to convert to array and process
        pm25_array = np.asarray(pm25)
        return np.array([convert_single_pm25(float(val)) if not np.isnan(val) else np.nan 
                        for val in pm25_array.flatten()]).reshape(pm25_array.shape)
...
def convert_aqi_dataframe_columns(
    ...
    if indoor_aqi_col in df.columns:
        df[indoor_pm25_col] = df[indoor_aqi_col].apply(
    ...
    if outdoor_aqi_col in df.columns:
        df[outdoor_pm25_col] = df[outdoor_aqi_col].apply(
            lambda x: aqi_to_pm25(x) if pd.notna(x) and x >= 0 else np.nan
        )
...
def calculate_indoor_outdoor_ratio(
    indoor_pm25: Union[float, np.ndarray],
    outdoor_pm25: Union[float, np.ndarray],
    min_outdoor: float = 1.0
...
def clean_timestamp_data(
    ...
    if start_date is not None:
        df = df[df[timestamp_col] >= start_date].copy()
    
    if end_date is not None:
        df = df[df[timestamp_col] <= end_date].copy()
...
def is_nighttime(timestamp: datetime, 
                night_start_hour: int = 22, 
                night_end_hour: int = 8) -> bool:
    """
...
def filter_nighttime_data(
    ...
    night_mask = df[timestamp_col].apply(
        lambda ts: is_nighttime(ts, night_start_hour, night_end_hour)
    )
    
    night_data = df[night_mask].copy()
    
    logger.info(f"Filtered to {len(night_data)} nighttime measurements "
               f"from {len(df)} total measurements")
