...
class KalmanFilterTracker(BaseFilterTracker):
    ...
    def _predict_step(self, dt_hours: float, outdoor_pm25: float) -> float:
        ...
        if self.prev_indoor is not None:
            ...
            predicted_indoor = self._temporal_model(
                self.prev_indoor, outdoor_pm25, dt_hours
            )
        else:
    ...
    def _update_step(self, observed_indoor: float, outdoor_pm25: float, 
                    predicted_indoor: float, dt_hours: float, confidence_factor: float = 1.0) -> None:
    ...
    def _finalize_daily_data(self, date) -> None:
        ...
        if len(day_history) >= 3:
            daily_record = {
                ...
                'prediction_error': float(np.mean([abs(h['actual_indoor'] - h['predicted_indoor']) for h in day_history])),
    ...
    def plot_efficiency_trend(self, days_back: Optional[int] = None, save_path: Optional[str] = None) -> None:
        ...
        axes[2].plot(daily_df['timestamp'], daily_df['prediction_error'], 'purple', linewidth=2, marker='o', markersize=3)
