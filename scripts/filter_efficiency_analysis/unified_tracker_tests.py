...
class UnifiedTester:
    ...
    def run_scenario(self, scenario_name: str, measurements: List[Dict], 
                    scenario_info: Dict, models_to_test: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for model_name in models_to_test:
            ...
            try:
                ...
                performance = self._calculate_performance_metrics(
                    model, measurements, scenario_info
                )
    ...
    def _calculate_performance_metrics(self, model: BaseFilterTracker, 
                                     measurements: List[Dict], 
                                     scenario_info: Dict) -> Dict[str, Any]:
    ...
    def create_comparison_plots(self, scenario_name: str) -> List[str]:
        ...
        viz = FilterVisualization(str(self.output_dir))
        
        # Create plots for each successful model
        saved_files = []
        successful_models = {
            name: result for name, result in scenario_data['model_results'].items() 
...
def main():
    ...
    scenarios = [
        ...
        ('step_changes', generator.generate_complete_dataset(
            scenario='step_test',
            days=7  # 7 days = ~3 steps of 2 days each with some extra
        )),
        
        # Filter degradation (simulate with gradually changing efficiency)
        ('filter_degradation', generator.generate_complete_dataset(
            scenario='degraded_filter',  # Use existing degraded scenario
            days=30
        )),
        
        # Noisy measurements scenario
        ('noisy_data', generator.generate_complete_dataset(
