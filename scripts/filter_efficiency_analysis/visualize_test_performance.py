#!/usr/bin/env python3
"""
Visualization script for analyzing filter efficiency test performance.

This script generates synthetic test data, runs the analysis, and creates
comprehensive visualizations to assess model performance.
"""
...
sys.path.append(str(script_dir / 'tests'))
...
class TestPerformanceVisualizer:
    ...
    def run_comprehensive_analysis(self, days: int = 14, random_seed: int = 42) -> Dict[str, Any]:
        ...
        for scenario in scenarios:
            ...
            try:
                ...
                analyzer._load_data = lambda days_back: dataset
                ...
                if analysis_results.get('success'):
                    ...
                    print(f"  ✓ Success: Estimated efficiency = {perf['efficiency_percentage']:.1f}% "
    ...
    def create_time_series_plots(self):
        ...
        for idx, (scenario, data) in enumerate(self.results.items()):
            if not data.get('success'):
                axes[idx].text(0.5, 0.5, f"{scenario}: Analysis Failed", 
            ...
            # Plot time series
            axes[idx].plot(dataset['timestamp'], dataset['outdoor_pm25'], 
                          label='Outdoor PM2.5', color='red', alpha=0.7, linewidth=1)
            axes[idx].plot(dataset['timestamp'], dataset['indoor_pm25'], 
                          label='Indoor PM2.5', color='blue', alpha=0.8, linewidth=1)
    ...
    def create_scatter_plots(self):
        ...
        for idx, (scenario, data) in enumerate(self.results.items()):
            ...
            if not data.get('success'):
                axes[idx].text(0.5, 0.5, f"Analysis Failed", 
                              ha='center', va='center', transform=axes[idx].transAxes,
                              fontsize=12, color='red')
                axes[idx].set_title(f"{scenario.replace('_', ' ').title()}")
                continue
            ...
            # Create scatter plot
            axes[idx].scatter(dataset['outdoor_pm25'], dataset['indoor_pm25'], 
            ...
            axes[idx].plot(outdoor_range, theoretical_indoor, 'g-', 
                          label=f'True model (η={true_eff:.2f})', linewidth=2)
            ...
            if data['analysis_results']['success']:
                ...
                axes[idx].plot(outdoor_range, estimated_indoor, 'm--', 
                              label=f'Estimated (η={est_eff:.2f})', linewidth=2)
    ...
    def create_parameter_comparison(self):
        ...
        for i, scenario in enumerate(scenarios):
            axes[1,0].annotate(scenario, (true_efficiencies[i], estimated_efficiencies[i]),
...
def main():
    ...
    results = visualizer.run_comprehensive_analysis(days=14, random_seed=42)
