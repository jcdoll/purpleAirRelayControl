...
def test_erv_functions_match_original_when_erv_disabled():
    ...
    assert (
        abs(original_indoor - erv_indoor) < 0.001
    ), f"Indoor PM2.5 should match: {original_indoor:.3f} vs {erv_indoor:.3f}"
    ...
    assert (
        abs(original_efficiency - erv_efficiency) < 0.001
    ), f"Filter efficiency should match: {original_efficiency:.3f} vs {erv_efficiency:.3f}"
...
def test_erv_increases_indoor_pm25():
    ...
    assert (
        indoor_with_erv > indoor_no_erv
    ), f"ERV should increase indoor PM2.5: {indoor_with_erv:.3f} > {indoor_no_erv:.3f}"
...
def test_erv_affects_filter_efficiency_estimation():
    ...
    assert (
        error_with_erv < error_no_erv
    ), f"ERV-aware method should be more accurate: {error_with_erv:.3f} < {error_no_erv:.3f}"
    assert estimated_efficiency_no_erv < true_filter_efficiency, "Ignoring ERV should underestimate filter efficiency"
    assert (
        abs(estimated_efficiency_with_erv - true_filter_efficiency) < 0.05
    ), "ERV-aware method should be within 5% of true efficiency"
...
def test_calculate_infiltration_components():
    ...
    assert (
        components_no_erv['total_ach'] == components_no_erv['natural_ach']
    ), "Total should equal natural when ERV disabled"
...
def test_erv_mass_balance_consistency():
    ...
    assert (
        abs(io_ratio - calculated_ratio) < 0.001
    ), f"I/O ratios should match: {io_ratio:.3f} vs {calculated_ratio:.3f}"
    ...
    assert (
        abs(solved_efficiency - filter_efficiency) < 0.001
    ), f"Solved efficiency should match: {solved_efficiency:.3f} vs {filter_efficiency:.3f}"
