...
def test_zero_confidence_no_crash():
    ...
    for i in range(5):
        tracker.add_measurement(
            day_timestamp + timedelta(minutes=i * 10), indoor_pm25=20.0 + i, outdoor_pm25=50.0 + i * 2
        )

    # Add nighttime measurements (normal confidence)
    for i in range(5):
        tracker.add_measurement(
            night_timestamp + timedelta(minutes=i * 10), indoor_pm25=15.0 + i, outdoor_pm25=45.0 + i * 2
        )
