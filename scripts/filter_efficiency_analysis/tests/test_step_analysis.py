"""
Step test analysis as part of the standard pytest suite.

These tests validate filter efficiency estimation using clean, controlled 
...
def generate_step_test_data(
    ...
    sampled_outdoor = np.interp(
        [t.timestamp() for t in time_index],
        outdoor_timestamps,
        outdoor_values
    )
    ...
    scenario_info = {
        ...
        'total_ach': (building_params['infiltration_ach'] + 
                     building_params['hvac_m3h'] / building_params['volume_m3'] + 
                     building_params['deposition_ach']),
        'time_constant_hours': 1.0 / (building_params['infiltration_ach'] + 
                                     building_params['hvac_m3h'] / building_params['volume_m3'] + 
                                     building_params['deposition_ach'])
    }
...
@pytest.mark.parametrize("filter_efficiency,expected_accuracy", [
...
def test_step_analysis_filter_efficiency(filter_efficiency: float, expected_accuracy: float):
    ...
    assert confidence >= 0.5, (
        f"Confidence {confidence:.2f} too low for step test (uncertainty: {summary.get('efficiency_uncertainty', 'N/A')})"
    )
