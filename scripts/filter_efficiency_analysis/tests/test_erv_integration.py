...
def test_erv_adds_to_natural_infiltration():
    ...
    assert (
        abs(total_with_erv - expected_total) < 0.001
    ), f"Total should be {expected_total:.3f}, got {total_with_erv:.3f}"
...
def test_erv_different_flow_rates():
    ...
    assert (
        abs(erv_200_cfm - erv_100_cfm * 2.0) < 0.001
    ), f"200 CFM should be 2x 100 CFM: {erv_200_cfm:.3f} vs {erv_100_cfm * 2:.3f}"
...
def test_erv_realistic_example():
    ...
    print("Lifebreath 170 ERVD Analysis:")
