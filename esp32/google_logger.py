...
class GoogleFormsLogger:
    ...
    def log(self, outdoor_aqi, indoor_aqi, switch_state, vent_state, reason):
        ...
        try:
            ...
            form_data = {
                'entry.1205406470': (
                    str(int(outdoor_aqi)) if outdoor_aqi > 0 else 'N/A'
                ),  # Outdoor AQI
                'entry.1516636704': (
                    str(int(indoor_aqi)) if indoor_aqi > 0 else 'N/A'
                ),  # Indoor AQI
            ...
            if success:
                ...
                if config.DEBUG_MODE:
                    print(
                        f"Logged to Google Forms: Out={outdoor_aqi:.0f}, In={indoor_aqi:.0f}, State={vent_state}"
                    )
            else:
                if config.DEBUG_MODE:
